...
# Try seeing if we can detect all combinations of numerical variables with categorical variables.
  num.plot <- ggplot(DATA, aes(x = RESIDENTIAL.UNITS, y = GROSS.SQUARE.FEET, color = BOROUGH.COMBINED)) +
    geom_point() + 
    geom_smooth(method = "lm", se = FALSE)
  print(num.plot)

# We can see that an interaction effect may be unlikely but its hard to tell.
```